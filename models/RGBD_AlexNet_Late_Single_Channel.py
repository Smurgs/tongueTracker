...
def get_model_name(): return 'RGBD_AlexNet_Late_Single_Channel2'
        ...
        with tf.variable_scope('rgb_model'):
            with tf.variable_scope('conv_variables'):
                with tf.device('/cpu:0'):
                    ...
                    conv1W = tf.get_variable('conv1W', shape=[11, 11, 3, 96],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
                    conv1B = tf.get_variable('conv1B', shape=[96], initializer=tf.zeros_initializer())
                    conv2W = tf.get_variable('conv2W', shape=[5, 5, 96, 256],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
                    ...
                    conv4W = tf.get_variable('conv4W', shape=[3, 3, 384, 384],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
                    conv4B = tf.get_variable('conv4B', shape=[384], initializer=tf.zeros_initializer())
                    conv5W = tf.get_variable('conv5W', shape=[3, 3, 384, 256],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
                    conv5B = tf.get_variable('conv5B', shape=[256], initializer=tf.zeros_initializer())

            # Build graph
            with tf.variable_scope('conv1'):
                rgb_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(rgb_x, conv1W, [1, 4, 4, 1], 'VALID'), conv1B))
                rgb_out = tf.nn.max_pool(rgb_out, [1, 3, 3, 1], [1, 2, 2, 1], 'VALID')
                rgb_out = tf.layers.dropout(rgb_out, 0.1, training=training_ph)
            with tf.variable_scope('conv2'):
                rgb_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(rgb_out, conv2W, [1, 1, 1, 1], 'SAME'), conv2B))
                rgb_out = tf.nn.max_pool(rgb_out, [1, 3, 3, 1], [1, 2, 2, 1], 'VALID')
                rgb_out = tf.layers.dropout(rgb_out, 0.1, training=training_ph)
            with tf.variable_scope('conv3'):
                rgb_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(rgb_out, conv3W, [1, 1, 1, 1], 'SAME'), conv3B))
                rgb_out = tf.layers.dropout(rgb_out, 0.1, training=training_ph)
            with tf.variable_scope('conv4'):
                rgb_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(rgb_out, conv4W, [1, 1, 1, 1], 'SAME'), conv4B))
                rgb_out = tf.layers.dropout(rgb_out, 0.1, training=training_ph)
            with tf.variable_scope('conv5'):
                rgb_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(rgb_out, conv5W, [1, 1, 1, 1], 'SAME'), conv5B))
                rgb_out = tf.nn.max_pool(rgb_out, [1, 3, 3, 1], [1, 2, 2, 1], 'VALID')
            rgb_out = tf.contrib.layers.flatten(rgb_out)

        with tf.variable_scope('depth_model'):
            with tf.variable_scope('conv_variables'):
                with tf.device('/cpu:0'):
                    reg = tf.contrib.layers.l2_regularizer(scale=0.15)
                    conv1W = tf.get_variable('conv1W', shape=[11, 11, 1, 96],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
                    conv1B = tf.get_variable('conv1B', shape=[96], initializer=tf.zeros_initializer())
                    conv2W = tf.get_variable('conv2W', shape=[5, 5, 96, 256],
                                             initializer=tf.contrib.layers.xavier_initializer(), regularizer=reg)
            ...
            # Build graph
            with tf.variable_scope('conv1'):
                depth_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(depth_x, conv1W, [1, 4, 4, 1], 'VALID'), conv1B))
                depth_out = tf.nn.max_pool(depth_out, [1, 3, 3, 1], [1, 2, 2, 1], 'VALID')
                depth_out = tf.layers.dropout(depth_out, 0.1, training=training_ph)
            with tf.variable_scope('conv2'):
            ...
            with tf.variable_scope('conv3'):
                ...
                depth_out = tf.layers.dropout(depth_out, 0.1, training=training_ph)
            with tf.variable_scope('conv4'):
                depth_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(depth_out, conv4W, [1, 1, 1, 1], 'SAME'), conv4B))
                depth_out = tf.layers.dropout(depth_out, 0.1, training=training_ph)
            with tf.variable_scope('conv5'):
                depth_out = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(depth_out, conv5W, [1, 1, 1, 1], 'SAME'), conv5B))
                depth_out = tf.nn.max_pool(depth_out, [1, 3, 3, 1], [1, 2, 2, 1], 'VALID')
            depth_out = tf.contrib.layers.flatten(depth_out)

        model_out = tf.concat([rgb_out, depth_out], axis=-1)
